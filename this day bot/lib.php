...
class MediaWikiApi {
    ...
    function editPage($pageName, $content, $createonly = false, $prepend = false, $append = false) {
        ...
        if ($append)
            $url .= "&appendtext=true";
        $data = httpRequest($url, $params = "format=xml&action=edit&title=$pageName&text=$content&token=$editToken&bot");
        $xml = simplexml_load_string($data);
...
function httpRequest($url, $post = "", $retry = false, $retryNumber = 0, $headers = array()) {
    ...
    try {
        ...
        curl_setopt($ch, CURLOPT_USERAGENT, 'FO-BoT (FO-nTTaX operates this yada yada yada) - Hi r1ch :D');
